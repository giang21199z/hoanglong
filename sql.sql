...
CREATE TABLE IF NOT EXISTS `hoanglong_web`.`category_news` (
  ...
  PRIMARY KEY (`idcategory`, `parent`),
  INDEX `fk_category_news_category_news1_idx` (`parent` ASC),
  CONSTRAINT `fk_category_news_category_news1`
    FOREIGN KEY (`parent`)
    REFERENCES `hoanglong_web`.`category_news` (`idcategory`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `hoanglong_web`.`service` (
  ...
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `category_service_idcategory` INT(11) NOT NULL,
  PRIMARY KEY (`idnews`, `category_service_idcategory`),
  INDEX `fk_service_category_service1_idx` (`category_service_idcategory` ASC),
  CONSTRAINT `fk_service_category_service1`
    FOREIGN KEY (`category_service_idcategory`)
    REFERENCES `hoanglong_web`.`category_service` (`idcategory`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
